...
class Notifier:
    ...
    def notify_github_report(self, repo, report):
        ...
        if self.email_settings:
            ...
            LOG.warning("邮件设置未配置正确，无法发送 GitHub 通知")
    ...
    def notify_hn_report(self, date, report):
        ...
        """
        if self.email_settings:
            subject = f"[Hacker News] {date} 今日技术前沿趋势"
            self.send_email(subject, report)
        else:
            LOG.warning("邮件设置未配置正确，无法发送 Hacker News 通知")
    
    def send_email(self, subject, report):
        LOG.info(f"开始发送邮件通知: {subject}")
        msg = MIMEMultipart()
        msg['From'] = self.email_settings['from']
        msg['To'] = self.email_settings['to']
        msg['Subject'] = subject
        
        # 将 Markdown 内容转换为html格式
